...
class PyaudioRecordingSettings:
    ...
    The frames per buffer requires the sample rate to be translated to ms. e.g.
...
class PyaudioRecorder(ports.Recorder):
    ...
    def record(self, duration: float, recording_rate: int) -> pathlib.Path:
        file_path = self.temp_path / "temp.wav"
        stream = self._create_stream()
        frames = self._record(
            stream=stream,
            recording_rate_khz=self.settings.frames_per_buffer,
            duration=duration,
        )
        stream.stop_stream()
        stream.close()
    ...
    def _create_stream(self):
        return self.audio_object.open(
            ...
            rate=self.settings.frames_per_buffer,
            input=True,
            frames_per_buffer=self.settings.frames_per_buffer,
        )

    def _record(
        self, stream: pyaudio.Stream, recording_rate_khz: int, duration: float
    ) -> list[bytes]:
        """
        Record audio and save it to the path
        """
        ...
        frames = []
        for i in range(
            0, int(recording_rate_khz / self.settings.frames_per_buffer * duration)
        ):
    ...
    def _write_to_file(self, file_path: pathlib.Path, frames: list[bytes]):
        ...
        waveFile.setframerate(self.settings.frames_per_buffer)
